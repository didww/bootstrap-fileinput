/*!
 * DIDWW-fork
 * https://github.com/didww/bootstrap-fileinput
 *
...
(function (factory) {
        ...
        _initXhr: function (xhrobj, previewId, fileCount) {
            ...
            if (xhrobj.upload) {
                xhrobj.upload.addEventListener('progress', function (event) {
                    ...
                    var pct_limit = self.progressUploadFreezeThreshold;
                    if (self.progressUploadFreezeThreshold && pct > pct_limit) {
                        if (!isEmpty(self.progressTemplate)) {
                            self.$progress.html(
                                self.progressTemplate
                                    .replace('{percent}', pct_limit)
                                    .replace('{percent}', pct_limit)
                                    .replace('{percent}%', self.msgFreezeThreshold)
                            );
                        }
                    } else {
                        if (previewId) {
    ...
    $.fn.fileinput.defaults = {
        ...
        progressUploadFreezeThreshold: 95,
